...
export default class RiwayatPage {
    ...
    return `
      <section class="background-gradient d-flex justify-content-center min-vh-100 pt-5 pt-lg-4">
        <article class="mt-5 card bg-cream container-fluid m-3 m-md-5 shadow-lg p-2 p-lg-5">
          ...
          <div id="riwayat-loading" class="d-flex align-items-center justify-content-center">
  ...
  async afterRender() {
    ...
    document.querySelectorAll('.modal').forEach((modal) => {
      modal.addEventListener('hide.bs.modal', () => {
        document.getElementById('riwayat-body').focus();
      });
    });
  }

  getDataRiwayatSuccessfully(data_response) {
    const bodyRiwayat = document.getElementById('riwayat-body');
    const data_riwayat = data_response.data;

    if (data_riwayat.length > 0) {
      bodyRiwayat.innerHTML = data_riwayat.map((data, index) => `
        <div class="card shadow riwayat-item-card" data-bs-toggle="modal" data-bs-target="#modalDetailImg${index}">
          <img id="img-${index}" src="${CONFIG.BASE_URL + data.image}" class="object-fit-cover rounded h-100 img-item-riwayat d-none" alt="Foto Hewan ${data.nama_hewan}">
          <div id="loading-image-${index}" class="loading-image-riwayat">
      ...
      document.querySelectorAll('.img-item-riwayat').forEach((item, index) => {
      ...
      riwayatLoading.innerHTML = `
        <p class="loading-text fs-1">
          ...
          <span>Data Riwayat Belum Tersedia</span>
        </p>
      `;
    }
  }
  ...
  _setPagination(data) {
    ...
    limitData.addEventListener('change', async () => {
      await this.#presenter.getDataRiwayat(1, limitData.value);
    ...
    paginatePrev.addEventListener('click', async (e) => {
      e.preventDefault();
      await this.#presenter.getDataRiwayat(data.page - 1, limitData.value);
      scrollTo({ top: 0, behavior: 'smooth' });
    });
    ...
    paginateLast.addEventListener('click', async (e) => {
      e.preventDefault();
      await this.#presenter.getDataRiwayat(totalPages, limitData.value);
      scrollTo({ top: 0, behavior: 'smooth' });
    });
    ...
    paginateNumberFirst.addEventListener('click', async (e) => {
      ...
      await this.#presenter.getDataRiwayat(page, limitData.value);
      scrollTo({ top: 0, behavior: 'smooth' });
    })
  ...
  getDataRiwayatFailed(error) {
    ...
    Toast.fire({
      icon: "error",
      title: 'Terjadi kesalahan saat memuat riwayat!',
    });
